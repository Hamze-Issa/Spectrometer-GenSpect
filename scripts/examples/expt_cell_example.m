% EXPT_CELL_EXAMPLE is a demonstration script file for the GENSPECT 
% line-by-line radiative transfer code which generates lines data and 
% radiance for a Sun-like source viewed through a laboratory cell
...
cell_order = [1];
ang_size=1;
paths1=path_atm(cells,1,gases_to_use,cells.mixrat(cell_order,gases_to_use),theta,ang_size,paths);
paths1=radiance(paths1,1,'by path');
r1=paths1.radiance;
...
% Plot results
figure(1)
plot(paths.wavnum,r1,'b'),grid,hold
plot(paths.wavnum,r2,'g')
plot(paths.wavnum,r1-r2,'r')
